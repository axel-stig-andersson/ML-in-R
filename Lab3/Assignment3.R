...
winit <- runif(10,-1,1)
# h <- rep(10,10)
  nn <- neuralnet(sin~., data=train, hidden=10, threshold=0.01, startweights = winit)
...
plot(train, cex=2)
points(test, col = "blue", cex=1)
points(test[,1],predict(nn.ReLU,test), col="green", cex=1)
...
plot(mydata2, cex=2, ylim=c(-9,9))
points(mydata2[,1],predict(nn,mydata2), col="green", cex=1) 
...
nn_revert <- neuralnet(Var~., data = mydata3, hidden=10, threshold=0.01, startweights = winit, stepmax = 1e+09)
    

plot(mydata3, cex=2)
points(mydata3[,1],predict(nn_revert,mydata3), col="green", cex=1) 
