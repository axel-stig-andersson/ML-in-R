...
# data.scaled <- as.data.frame(cbind(scale(data[,-101]), data[,101]))
# attach(data.scaled)
...
plot(u[, 1], main = "Traceplot, PC1", col = "lightgreen", pch = 8, 
     ylab = "Weight")
points(index.top.5, u[index.top.5, 1], col = "red", pch = 20)

# install.packages("ggfortify")
library(ggfortify)
autoplot(res.2, colour = "ViolentCrimesPerPop", label = F, label.size = 2, 
...
#train <- as.data.frame(cbind(scale(select(train, -ViolentCrimesPerPop)), 
#                            ViolentCrimesPerPop))
# Test
...
#test <- as.data.frame(cbind(scale(select(test, -ViolentCrimesPerPop)), 
#                           ViolentCrimesPerPop))
...
plot(test$ViolentCrimesPerPop,pred.test, col="lightgreen", pch=20, ylim = c(-1,3), xlab = "Actual", ylab = "Predicted")
...
plot(multi.mse.train[500:21000], col = "lightgreen", main = "OPTIM MSE", pch = 20,
     ylab = "MEAN SQUARE ERROR", xlab = "ITERATIONS", ylim = c(0, 0.8), type = "p")
points(multi.mse.test[500:21000], col = "lightblue", main = "OPTIM MSE", pch = 20,
       type = "p")
points(stopping.opt.test - 500, multi.mse.test[stopping.opt.test], col = "black")
points(stopping.opt.train - 500, multi.mse.train[stopping.opt.train], col = "black")
legend("topright",
       legend = c("TRAIN", "TEST"),
